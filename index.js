...
exports.initialize = function (connection) {
  try {
    ...
    if (ex.name === 'MissingSchemaError') {
      ...
      counterSchema.index({ field: 1, groupingField: 1, model: 1 }, { unique: true, required: true, index: -1 });
        ...
        function (err, counter) {
          if (!counter) {
            ...
            counter = new IdentityCounter({ 
              model: settings.model, 
              field: settings.field, 
              groupingField: doc[settings.groupingField] || '', 
              count: settings.startAt - settings.incrementBy 
            });
        ...
        if (ready) {
          ...
          if (typeof doc[settings.field] === 'number') {
            IdentityCounter.findOneAndUpdate(
              ...
              { model: settings.model, field: settings.field, groupingField: doc[settings.groupingField], count: { $lt: doc[settings.field] } },
